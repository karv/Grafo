...
namespace CE.Graph.Grafo.Estáticos
{
	...
	/// An unweighted graph.
	/// </summary>
	public class Graph<T> : CommonGraph<T>, IGraph<T>
	{
		/// <summary>
		...
		public Graph (IGraph<T> graf, bool sóloLectura = true)
		...
		public Graph<T> Clonar (bool sóloLectura = false)
		{
			var ret = new Graph<T> (NodeArray, IsSymmetric, sóloLectura);
			for (int i = 0; i < NodeCount; i++)
				for (int j = 0; j < (IsSymmetric ? i + 1 : NodeCount); j++)
				{
		...
		public Graph<T> AsReadonly () => Clonar (true);

		/// <summary>
		/// Gets a subgraph generated by the specified node collection.
		/// </summary>
		public Graph<T> GetSubgraph (ICollection<T> nodeSubset)
		{
			var ret = new Graph<T> (nodeSubset, IsSymmetric, IsReadOnly);
